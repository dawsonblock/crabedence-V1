...
describe("PostgresCoordinatorStorage", () => {
  ...
  it("acquires and releases a session advisory lock", async () => {
    ...
    expect(clientQuery).toHaveBeenCalledWith(expect.stringContaining("pg_try_advisory_lock"), [
      "crabbox.coordinator",
    ]);

    await lock!.release();
    expect(clientQuery).toHaveBeenCalledWith(expect.stringContaining("pg_advisory_unlock"), [
      "crabbox.coordinator",
    ]);
    expect(release).toHaveBeenCalledTimes(1);

    // The lease is free for a subsequent holder (e.g., the replacement process).
    await expect(storage.acquireCoordinatorLock()).resolves.toBeDefined();
  });

  it("returns undefined and frees the client when the lock is contended", async () => {
    const release = vi.fn<() => void>();
    const client = {
      query: vi.fn<() => Promise<QueryResult<QueryResultRow>>>(async () =>
    ...
    await expect(storage.acquireCoordinatorLock()).resolves.toBeUndefined();
    expect(release).toHaveBeenCalledTimes(1);
  });

  it("releases the advisory lock on close", async () => {
    const clientQuery = vi.fn<() => Promise<QueryResult<QueryResultRow>>>(async () =>
    ...
    await storage.acquireCoordinatorLock();
    await storage.close();
