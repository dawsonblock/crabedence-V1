...
export interface RunEvidenceV1 {
  ...
  // Outcome
  exit_code: number;
  run_status: "succeeded" | "failed" | "timed-out" | "canceled";
  error_kind?: "" | "command-exit" | "timeout" | "canceled" | "provider-error";
  command_text?: string;
  ...
  // Sync metadata
  sync_delegated?: boolean;
  sync_skipped?: boolean;
  sync_mode?: string;
  sync_transfer_files?: number;
  sync_transfer_bytes?: number;
  sync_fallback_reason?: string;

  // Phases
  runner_phases?: RunnerPhaseEntry[];
  sync_phases?: TimingPhaseEntry[];
  command_phases?: TimingPhaseEntry[];
...
export interface TimingPhaseEntry {
  name: string;
  ms?: number;
  skipped?: boolean;
  reason?: string;
