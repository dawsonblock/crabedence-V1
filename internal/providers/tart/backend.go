...
type backend struct {
	...
	supervisor            shared.ProcessSupervisor
}
...
func newBackend(spec ProviderSpec, cfg Config, rt Runtime) Backend {
	...
	b.supervisor = &tartProcessSupervisor{backend: b}
	return b
}

// newBackendWithSupervisor is like newBackend but injects a ProcessSupervisor.
// Tests use this to substitute a fake without spawning real tart processes.
func newBackendWithSupervisor(spec ProviderSpec, cfg Config, rt Runtime, sup shared.ProcessSupervisor) Backend {
	applyDefaults(&cfg)
	b := &backend{
		spec:                  spec,
		cfg:                   cfg,
...
func (b *backend) startSupervisedVM(ctx context.Context, name string, keep bool) (*startupProcess, error) {
	handle, err := b.supervisor.Start(ctx, shared.ProcessStartRequest{
		Name:           name,
		Keep:           keep,
		ObserveTimeout: b.startupObserveTimeout,
	})
	if err != nil {
		return nil, err
	}
...
func (b *backend) Acquire(ctx context.Context, req AcquireRequest) (target LeaseTarget, acquireErr error) {
	...
	startup, err := b.startSupervisedVM(ctx, name, req.Keep)
	if err != nil {
		return LeaseTarget{}, errors.Join(err, cleanupUnclaimedVM())
	}
