// Signer policy qualification: production mode and multi-replica
// deployments must refuse to auto-generate a signing identity. The
// trust root is provisioned, never silently created per host —
// otherwise each instance would mint receipts its peers cannot
...
func TestEvidenceKeyPolicyDevelopmentAllowsGeneration(t *testing.T) {
	...
	if err := validateEvidenceKeyPolicy(""); err != nil {
		t.Fatalf("development must allow missing key path, got %v", err)
	}
	if err := validateEvidenceKeyPolicy(filepath.Join(t.TempDir(), "does-not-exist.pem")); err != nil {
...
func TestEvidenceKeyPolicyProductionRequiresProvisionedKey(t *testing.T) {
	...
	if err := validateEvidenceKeyPolicy(missing); err == nil {
	...
	if err := validateEvidenceKeyPolicy(existing); err != nil {
