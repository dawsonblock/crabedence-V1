...
- Evidence: add cross-language golden fixture tests — Go generates `testdata/evidence-golden.json` (six cases: basic, HTML-significant characters, non-ASCII Unicode, nested phases/artifacts, integers at the IEEE-754 safe boundary, and a minimal record with omitted fields), and both Go (`TestEvidenceGoldenFixtureValidates`, `TestEvidenceGoldenFixtureCanonicalBytesMatchJS`) and TypeScript (`run-evidence.test.ts`, including tampering, digest-format, binding, and oversize rejection tests) validate it through their respective validators, ensuring wire-format interoperability.
