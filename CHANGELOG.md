...
- Tests: live coverage for `RecordProviderObservation` (IN_FLIGHT and UNKNOWN paths), `EnterRecoveryWithObservation`, expired pre-dispatch normalization, stale locator scrubbing, signed CRITICAL finalization/recovery, and the ambiguous-column claim queries the old gate never ran.
