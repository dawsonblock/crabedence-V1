...
- Evidence: fix cross-language wire contract — Go and TypeScript now share a single canonicalization (code-point sorted JSON keys) and digest format (raw 64-char hex, no `sha256:` prefix). Previously Go used struct-order JSON with raw hex, while TypeScript used `localeCompare`-sorted JSON with `sha256:`-prefixed digests, making the two implementations mathematically incompatible.
...
- Evidence: add cross-language golden fixture tests — Go generates `testdata/evidence-golden.json`, and both Go (`TestEvidenceGoldenFixtureValidates`) and TypeScript (`run-evidence.test.ts`) validate it through their respective validators, ensuring wire-format interoperability.
