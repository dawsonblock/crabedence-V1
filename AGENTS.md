...
- Bump every version-carrying file, not just the changelog: `VERSION` (the single version source), the `CHANGELOG.md` section heading, `worker/package.json`, both root entries in `worker/package-lock.json`, and `nemo/package.json`. `node scripts/verify-version-consistency.mjs --tag vX.Y.Z` must pass before tagging; CI and both release workflows enforce it. See the Release Checklist in `docs/operations.md`.
