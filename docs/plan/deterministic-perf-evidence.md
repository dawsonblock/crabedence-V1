...
resolves the workload, metric, evidence, and runtime preconditions in the
[tracking issue](https://github.com/openclaw/crabbox/issues/280). It builds on
the [earlier feature-contract change](https://github.com/openclaw/crabbox/pull/857);
...
crabbox run \
  ...
  -- sh -eu -c '
    ...
    crabbox-wasmtime-meter \
      ...
      --workload example-org/workflow-action-pin-tests/v1 \
      --budget 1000000000 \
...
```json
{
  ...
  "workload": {
    "name": "example-org/workflow-action-pin-tests/v1",
...
The full schema-versioned artifact is the independently validated input to CI
and gate decisions. Timing JSON may later include a compact, optional `fuel`
summary for observability, but that summary is non-authoritative and is not a
second evidence format.

For v1, a valid artifact projects to timing JSON in exactly this way:

- `fuel.measured = results[0].measured`;
- `fuel.unit = results[0].unit`, which must equal the literal `fuel`;
...
The `engine` expression above is the literal stable serialization: one `@`
between engine and version, then one `+` between version and revision. No other
artifact fields are projected in v1. In particular, `budget`, `exceeded`, and
`comparisonKey` remain only in the artifact.

The `fuel` field is absent when no valid evidence artifact exists. Schema
validation and every internal-consistency invariant above must succeed before
projection; a malformed or inconsistent artifact cannot produce a timing fuel
summary. Existing timing JSON consumers therefore remain backward compatible
when deterministic evidence is unavailable.
